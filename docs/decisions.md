...
src/
  ...
  platform/   what belongs to no feature: the API client, config,
              storage, the send queue, the browser-API hooks
```
...
| **`normalize` and `slugOf` belong in their own module.** They sit in `exercises/catalog.ts` because that is where slugs were first needed, so `gyms/` reaches into `exercises/` for them although they know nothing about either. The API already keeps them apart in `Slugs.java`; the mirror would be `platform/slugs.ts`, with the eleven cases copied from `SlugsTest` moving with it | A third caller appears, or the next change to `catalog.ts`. The pairing with `SlugsTest` is load-bearing, so the tests travel with the functions |
