...
* **Signing out waits for the queue to empty.** The queue does not record whose workouts it holds, and the owner comes from the token it sends them with, so on a shared phone whoever signed in next would send the previous person's workouts as their own, into their history for good. While anything is queued, "Sign out" is replaced by a line saying why; a refused workout can be discarded, so nothing keeps someone signed in forever. Recording the owner in each queued workout was rejected: it would leave workouts nobody present can send, with no good answer for what to do with them. Signing out also deletes the phone's copy of the exercise catalog, which holds that user's own exercises.
